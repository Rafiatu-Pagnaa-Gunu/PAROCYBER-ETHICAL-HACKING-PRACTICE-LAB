# PAROCYBER-ETHICAL-HACKING-PRACTICE-LAB
Network Scanning, Enumeration &amp; Password Cracking Labs

Network Scanning, Enumeration &
Password Cracking Labs
Practical Replication Report

ParoCyber Ethical Hacking Class
Compiled Assignment — Questions 1, 2, 3 & 4
2025
 
Question 1: Nmap Labs Replication

Host Discovery
It identifies live machines by finding active devices on a subnet using subnet notation, without performing any port scanning.
...
This scan teaches stealth scanning techniques and TCP handshake concepts by demonstrating how a scanner sends a SYN packet, receives a SYN-ACK response from an open port, and then replies with an RST packet instead of completing the full TCP connection.
...
Often slower than TCP scans because UDP does not have a formal handshake and relies on ICMP port unreachable messages to determine closed ports.
...
OS Fingerprinting
It identifies operating systems such as Linux, Windows, and Router OS, as well as kernel versions, through the process of TCP/IP stack fingerprinting.

Aggressive Scan
 
 
Question 2: Nmap Scripting Engine (NSE)
...
After running this command, we realized the HTTPS port was closed. We tried restarting Apache to ensure port 443 (HTTPS) becomes available for the scan. We ran several commands to that effect but the port was not available for the scan.

 
 
Question 3: Active Reconnaissance, Exploitation & Post-Exploitation
...
Platform: TryHackMe AttackBox (browser-based Kali Linux) — Room: Metasploit Introduction

Phase 1: Launching Metasploit
Opened the terminal on the AttackBox and launched the Metasploit Framework:
msfconsole
Metasploit loaded successfully showing version 6.4.55-dev with 2502 exploits available.
 

Phase 2: Searching for Exploits
Searched for the VSFTPD vulnerability:
search vsftpd
...
Selected the VSFTPD backdoor exploit using its index number:
use 1
This loaded the module:
...
After gaining root access via the VSFTPD backdoor, the following post-exploitation steps were demonstrated:
...
-	Open FTP ports expose services that can be exploited if running vulnerable software versions.
-	VSFTPD 2.3.4 is a known backdoored version — always verify software integrity.
-	Weak passwords stored as hashes can be cracked with tools like John the Ripper.
-	Root access enables full system compromise including data exfiltration.
-	SSH with cracked credentials allows persistent, hard-to-detect attacker access.
...
Both Kali Linux and Metasploitable2 were configured on the same VirtualBox network to allow communication between the machines. The IP address of Metasploitable2 was identified using:
ifconfig
The IP address obtained was: 192.168.56.102
 

Scanning and Enumeration
Nmap was used to scan the target machine and identify running services.
nmap -sV 192.168.56.102
Results showed open services including FTP, SSH, and HTTP. The FTP service version identified was: vsftpd 2.3.4
Password Cracking Using Hydra
Hydra was used to perform an online password attack against the FTP service.
...
This indicated that the brute-force attack was actively running.

Password Cracking Using John the Ripper
A demonstration hash file was created to test offline password cracking functionality.
...
Observations
-	Weak passwords can be cracked easily using common password dictionaries.
-	Older systems such as Metasploitable2 use deprecated SSH algorithms that may not be compatible with modern systems by default.
-	Hydra is effective for online brute-force password attacks against network services.
-	John the Ripper is effective for offline password cracking once hashes are obtained.
